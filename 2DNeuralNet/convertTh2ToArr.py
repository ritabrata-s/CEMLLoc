'''
Convert image data from a root 2D histogram into numpy array.
...
for hist in histos :
    ...
    # Get the image array
    content = np.zeros((hist.GetNbinsX(), hist.GetNbinsY(), 1), dtype='f')
    for ybin in range(1, hist.GetNbinsY() + 1):
        for xbin in range(1, hist.GetNbinsX() + 1):
            content[xbin - 1, ybin - 1, 0] = hist.GetBinContent(xbin, ybin)
...
#plt.imshow(content, interpolation='none')
#plt.show()

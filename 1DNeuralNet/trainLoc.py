...
Create and train a model for transient source localization, based on the
count distributions in the pixels of the detector.
...
def get_convnet_calo_model(out_shape=2, dropout=None):
    ...
    inputCalo    = keras.Input(shape=(inpShape, 1))
...
#     print(caloconlayer.shape[1],caloconlayer.shape[2],caloconlayer.shape[3])
    ...
    caloconlayer    = keras.layers.Conv1D(100, (inputCalo.shape[1]), activation="relu")(inputCalo)
    caloconlayer    = keras.layers.Dropout(dropout)(caloconlayer) if dropout else caloconlayer
    caloconlayer    = keras.layers.Flatten()(caloconlayer)
#     caloconlayer    = keras.layers.Flatten()(inputCalo)
